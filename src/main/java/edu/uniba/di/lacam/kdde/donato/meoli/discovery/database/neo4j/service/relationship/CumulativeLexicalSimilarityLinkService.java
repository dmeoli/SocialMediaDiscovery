...
public class CumulativeLexicalSimilarityLinkService extends CumulativeLinkService<CumulativeLexicalSimilarityLink> {
    ...
    public void cumulateLinks(Collection<? extends Link> temporalSubGraph) {
        cumulativeLinkRepo.saveAll(temporalSubGraph.parallelStream()
                .filter(link -> link.getClass().equals(LexicalSimilarityLink.class))
                ...
                .map(entry -> cumulativeLinkRepo.findByCumulativeUserFromNameAndCumulativeUserToName(
                        ...
                        .map(cumulativeLexicalSimilarityLink -> {
                            cumulativeLexicalSimilarityLink.updateCumulativeTemporalSubGraphsCounter(
                                    temporalSubGraphNumber, entry.getValue().size());
                            return cumulativeLexicalSimilarityLink;
                        }).orElseGet(() -> new CumulativeLexicalSimilarityLink(entry.getKey().getLeft(),
